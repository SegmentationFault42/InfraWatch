// src/services/ping/manual_test.ts
...
async function testPingValidIP() {
  ...
  assert(typeof res.latency === 'number' && res.latency > 0, 'Latency deve ser número positivo');
...
async function testPingRetries() {
  ...
  const res = await pingHost('192.0.2.123', { retries: 3, intervalMs: 500, timeoutMs: 500 });
  const elapsed = Date.now() - start;
  console.log(res);
  assert(res.alive === false, 'Esperava que IP inválido estivesse dead');
  assert(res.attempts === 4, 'Esperava 4 tentativas (1 + 3 retries)');
  assert(elapsed >= 1500, 'Esperava tempo total pelo menos igual a soma dos intervalos');
}

async function testPingIntervalBetweenRetries() {
  console.log('Test: respeitar intervalo entre tentativas');
  const res = await pingHost('192.0.2.123', { retries: 2, intervalMs: 500, timeoutMs: 500 });
  console.log(res);
...
async function testPingZeroRetries() {
  ...
  console.log(res);
  assert(res.alive === true || res.alive === false, 'Deve lidar com retries zero sem erro');
...
async function testPingNegativeRetries() {
  ...
  const res = await pingHost('8.8.8.8', { retries: -1, timeoutMs: 2000 });
  console.log(res);
  assert(res.alive === true || res.alive === false, 'Deve lidar com retries negativos sem erro');
}
...
async function testPingInvalidHostname() {
  ...
  const res = await pingHost('invalid-hostname-xyz123.example', { retries: 1, timeoutMs: 1000 });
  console.log(res);
...
async function testPingZeroInterval() {
  ...
  const res = await pingHost('192.0.2.123', { retries: 3, intervalMs: 0, timeoutMs: 500 });
...
async function testPingNegativeInterval() {
  ...
  const start = Date.now();
  const res = await pingHost('192.0.2.123', { retries: 3, intervalMs: -100, timeoutMs: 500 });
  const elapsed = Date.now() - start;
  console.log(res);
  assert(res.alive === false, 'Esperava que IP inválido estivesse dead');
  assert(res.attempts === 4, 'Esperava 4 tentativas (1 + 3 retries)');
  // Só verifica que o tempo não foi absurdamente longo (timeout * attempts é um mínimo)
  assert(
...
async function runAllTests() {
  try {
    ...
    console.log('Todos os testes essenciais e extremos rodaram sem erro.');
  } catch (err) {
