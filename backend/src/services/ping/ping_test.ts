...
async function runTests() {
  ...
  console.log(await pingMultipleHosts([validHost, validHost2, invalidHost, "192.0.2.123"]));
  ...
  console.log(await pingHost("", {} as PingOptions));

  // ==== 3. Testes de Desempenho ====
  console.log("\n🧪 TESTES DE DESEMPENHO\n");

  const manyHosts = [validHost, validHost2, "github.com", "cloudflare.com", "microsoft.com"];
  let start = nowMs();
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    console.log(await pingMultipleHosts([validHost, validHost2, invalidHost]));
  }
